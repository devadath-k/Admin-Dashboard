...
export default function NavBar() {
  ...
  const NavItems = (NavList) =>(
    NavList.map((item, index)=>(
      <div key={item.text} onClick={()=>setNavIndex(index)} className={index!=navIndex? "text-[#336F7E] text-[1.3rem] font-bold rounded-[25px] navItem h-[50px] p-[10px]": "text-white bg-[#336F7E] text-[1.3rem] font-bold rounded-[25px] h-[50px] p-[10px]"} >
        {item.text}
      </div>
      )
    )
    ...
    <nav>
      ...
      <div className="w-[50%]">
        ...
        <div className="w-[50%] relative">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-[100%] h-[100%]">
